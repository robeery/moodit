...
const _resultsPath = 'stage_cache_benchmark_results.json';

void main() {
  test('stage cache benchmark (host)', () async {
    final bytes = await _loadOrGenerateFixture();
    final results = runStageCacheBenchmark(fixtureBytes: bytes);

    await File(_resultsPath).writeAsString(
      const JsonEncoder.withIndent('  ').convert(results),
    );

    printResults('Stage cache benchmark (host)', results);
